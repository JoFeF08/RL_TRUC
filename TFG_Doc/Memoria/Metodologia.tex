...
l'agent aleatori i 100~partides contra l'agent basat en regles. Per evitar el biaix
posicional (ser \emph{mà} dona un lleuger
...
secció~\ref{sec:ppo}). El win-rate, en canvi, és una mètrica
...
Cada terme compleix una funció diferent. L'agent basat en regles (pes 0{,}75) és l'oponent
\textbf{discriminant}: batre una heurística coherent requereix estratègia real, de manera
que aquest terme és el que captura la qualitat de l'agent. L'agent aleatori
...
\subsection*{L'oponent de referència: l'agent basat en regles}

L'agent basat en regles és un agent
...
l'agent basat en regles una barrera molt més exigent del que sembla.
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    \bottomrule
  \end{tabular}
  \caption[Les sis variants de l'agent basat en regles.]{Les sis variants de l'agent basat en regles que formen el \emph{pool}
...
d'entrenament s'expressa en \emph{timesteps} (passos d'interacció amb l'entorn).
Al cos del capítol només se'n destaquen els hiperparàmetres rellevants per a cada decisió.

\input{MetodologiaBloc1}
\input{MetodologiaBloc2}
\input{MetodologiaBloc3}
