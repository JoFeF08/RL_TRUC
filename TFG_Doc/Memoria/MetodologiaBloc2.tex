...
\subsection{Fase 3: extractor de característiques COS}
\label{sec:fase3}

\begin{sloppypar}
La Fase~3 estudia \emph{CosMultiInput}, un extractor de característiques convolucional
propi (el \emph{cos}, o COS, de la xarxa: la seva columna vertebral de processament),
com a alternativa a l'extractor pla per defecte de \ac{SB3}.
...
dissenya \emph{CosMultiInput}, un
...
(\texttt{net\_arch=[256,256]}), de manera que \emph{CosMultiInput} substitueix
l'extractor per defecte de \ac{SB3} (\emph{CombinedExtractor}, que simplement aplana
l'observació) sense cap altre canvi. L'adaptador \emph{CosMultiInputSB3} (\texttt{RL/%
...
\begin{figure}[H]
  ...
  \caption[Arquitectura \emph{CosMultiInput} de dues branques.]{\emph{CosMultiInput}: arquitectura de dues branques. La branca CNN processa
...
entrenaments nous amb COS i dos amb \ac{MLP} sobre mans; les quatre sèries \ac{MLP} de control i
...
(taula~\ref{tab:fase3}, comparada gràficament per protocol i agent a la
figura~\ref{fig:ann-fase3-cos-mlp} de l'annex~\ref{ann:grafiques}). Només aporta una millora
clara al protocol de control per a
...
\begin{table}[H]
  ...
  \begin{tabular}{llrrr}
    ...
    \bottomrule
  \end{tabular}
  \caption[Win-rate de COS amb pesos aleatoris vs MLP (Fase~3).]{Pic del win-rate ponderat (\(\text{metric}\)) a 24M passos per a COS amb pesos
           aleatoris vs \ac{MLP}. El COS només supera clarament el \ac{MLP} al control de
           DQN-SB3; al protocol de mans (el millor en absolut) la diferència és marginal.}
  \label{tab:fase3}
\end{table}
...
\emph{El preentrenament supervisat.} La idea és donar al cos \emph{CosMultiInput}
...
aconseguir-ho, s'afegeix temporalment al cos el model \emph{ModelPreEntrenament}, que
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[bloc, right=1.4cm of ds,  fill=orange!10] (cos) {COS\\(cos de la xarxa)};
  ...
  \end{tikzpicture}
  \caption[Esquema del preentrenament supervisat (Fase~3.5).]{Esquema del preentrenament supervisat: els tres caps propaguen gradient al cos
...
\begin{table}[H]
  ...
  \begin{tabular}{llcl}
    ...
    \bottomrule
  \end{tabular}
  \caption{Caps de predicció del \emph{ModelPreEntrenament} per al preentrenament
           supervisat del cos \emph{CosMultiInput}.}
...
\begin{table}[H]
  ...
  \begin{tabular}{lrr}
    ...
    \bottomrule
  \end{tabular}
  \caption{Pic del win-rate ponderat (\(\text{metric}\)) al protocol de mans (24M passos)
           per als quatre règims d'inicialització del cos.}
  \label{tab:fase35}
\end{table}

\paragraph{Decisió.} El preentrenament supervisat aporta valor real, però només quan el
cos queda \textbf{congelat}. S'adopta el COS preentrenat i congelat com a base
...
de l'agent basat en regles amb estils contrastats (taula~\ref{tab:pool-variants}). La diversitat
...
oponent. El nou entorn \emph{TrucGymEnvSessio} envolta \texttt{TrucGymEnvMa} i
...
lleuger~$\rightarrow$~caps de política i valor. S'usa \emph{RecurrentPPO} (de
\emph{sb3-contrib}) amb \emph{retropropagació en el temps truncada}
...
\emph{Pressupost i avaluació.} Es comparen dos entrenaments nous amb les línies base de la Fase~3.5:
\textbf{F4-ablació} (PPO + COS congelat + pool, sense memòria) i \textbf{F4-complet}
(\emph{RecurrentPPO} amb \ac{LSTM} sobre el COS congelat + pool); la primera comparació
aïlla l'efecte del pool i la segona, l'efecte de la memòria. Cada entrenament nou s'entrena 12M
passos, la meitat que a la Fase~3.5, perquè la retropropagació en el temps encareix molt el
còmput. L'avaluació, a més del win-rate ponderat (\(\text{metric}\)) habitual, acumula el
win-rate \emph{per
...
(taula~\ref{tab:fase4}). La figura~\ref{fig:fase4-corbes} mostra l'evolució del win-rate
ponderat (\(\text{metric}\)); la figura~\ref{fig:fase4-adaptacio} detalla el win-rate per
posició dins la sessió, clau per a H2. L'annex~\ref{ann:grafiques} en resumeix el pic per
variant a la figura~\ref{fig:ann-fase4-barplot}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\textwidth]{figures/fase4/fase4_corbes.pdf}
  \caption[Evolució del win-rate, quatre variants de la Fase~4.]{Evolució del win-rate ponderat (\(\text{metric}\)) al llarg de l'entrenament
           per als quatre entrenaments de la Fase~4, tallats a 12M passos. Línies discontínues:
           línies base de la Fase~3.5 (sense pool); línies sòlides: entrenaments amb pool; gruix
           addicional: model amb \ac{LSTM}. La línia puntejada gris marca el 85\%.}
...
\begin{figure}[H]
  ...
  \caption[Win-rate per posició dins la sessió (Fase~4).]{Win-rate per posició dins la sessió (mà~1 a mà~5) per als dos entrenaments de la
...
\begin{table}[H]
  ...
  \begin{tabular}{llrr}
    \toprule
    Entrenament & Memòria & Pic \(\text{metric}\) (12M) & Temps \\
    \midrule
    ...
    \bottomrule
  \end{tabular}
  \caption[Resultats de la Fase~4 (12M passos).]{Resultats de la Fase~4 (pressupost igualat a 12M passos). DQN frozen mostra el
...
F4-complet assoleix un 82{,}0\% de \(\text{metric}\), \emph{4~punts per sota} de F4-ablació
...
La causa subjacent del fracàs de H1 i H2 podria ser el \textbf{desajust entre entrenament i
...
contra les sis variants de l'agent basat en regles, de manera que la seva política és
...
Aquesta robustesa, però, encara no s'ha quantificat: la \(\text{metric}\) clàssica
