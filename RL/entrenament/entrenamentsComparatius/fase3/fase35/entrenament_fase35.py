...
def _aplicar_variant(model, variant: str, pesos_cos: str | None, lr: float):
    ...
    if variant in ("frozen", "finetune"):
        ...
        print(f"[fase35] Pesos preentrenats carregats: {pesos_cos}")
    ...
    if variant == "frozen":
        ...
        print(f"[fase35] Cos congelat. Params entrenables: {n_ent:,}/{n_tot:,}")


def _dqn_sb3_fase35(save_dir: Path, timesteps: int, device, variant: str,
                    pesos_cos: str | None):
...
def _ppo_sb3_fase35(save_dir: Path, timesteps: int, device, variant: str,
...
def main():
    parser = argparse.ArgumentParser(description="Fase 3.5: Estratègia d'Inicialització del COS")
    ...
    if args.agent == "dqn_sb3":
        _dqn_sb3_fase35(save_dir, args.steps, device, args.variant, args.pesos_cos)
    else:
        _ppo_sb3_fase35(save_dir, args.steps, device, args.variant, args.pesos_cos,
